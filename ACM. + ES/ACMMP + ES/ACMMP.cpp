...
bool ShowDepthMap(const std::string &depth_path, const cv::Mat& _depth, float depth_min, float depth_max)
{
    ...
    double inv_max = 1 / (depth_max - depth_min + 1e-8);
    cv::Mat norm_dmb= depth - depth_min;
    ...
    int top = getMax10(Hist, depth.rows * depth.cols);
    int down = getMin10(Hist, depth.rows * depth.cols);
    double new_min = depth_min + (depth_max - depth_min) * ((float)top / 256.0);
    double new_max = depth_min + (depth_max - depth_min) * ((float)down / 256.0);
    for(int i = 0; i < depth.rows; ++i){
        for(int j = 0; j < depth.cols; ++j){
            const float val = depth.at<float>(i,j);
            if(val < new_min) {
    ...
    cv::Mat bgr_img = cv::Mat::zeros(depth.rows, depth.cols, CV_8UC3);
    Colormap2Bgr(temp, bgr_img, mask);
    cv::imwrite(depth_path, bgr_img);
    return true;
}
...
bool ShowNormalMap(const std::string &normal_path, const cv::Mat &_normal)
{
	...
	cv::Mat normalized_normal = _normal.clone();
