...
void ProcessProblem(const std::string &dense_folder, const std::vector<Problem> &problems, const int idx, bool geom_consistency, bool planar_prior, bool hierarchy, bool multi_geometrty=false)
{
    ...
    {
        std::string depth_img_path = result_folder + "/depth.jpg";
        std::string normal_img_path = result_folder + "/normal.jpg";
        ShowDepthMap(depth_img_path, depths, acmmp.GetMinDepth(), acmmp.GetMaxDepth());
		ShowNormalMap(normal_img_path, normals);
    }
